				...
				BEGIN
					...
					BEGIN 
						...
						BEGIN
							...
							SELECT @v_query = 'set IDENTITY_INSERT '  + @v_targetCabinet + '..QueueDefTable ON' + ' Insert into QueueDefTable ( ' + @v_colStrQueueDef +' ) Select ' + @v_colStrQueueDef + ' from ' +  @dblinkString + '.QueueDefTable WITH (NOLOCK) where QUEUEID = '  +  CONVERT(nvarchar(10),@v_queueId)
							print ('queuedefquery -- ')
							print(@v_query)
							EXECUTE (@v_query)
							IF(@@error <> 0)
