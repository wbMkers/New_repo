...
AS
	...
	BEGIN	
		select @lCursor_String = 'INSERT INTO ' + @lDBTargetCab + 'PDBFOLDER (PARENTFOLDERINDEX, NAME, OWNER, CREATEDDATETIME, REVISEDDATETIME,
					...
					Select ParentFolderIndex, Name, 1, CreatedDatetime, RevisedDateTime, AccessedDateTime,
		...
		If Exists (Select * FROM SysObjects (NOLOCK) Where xType = 'U' and upper(name) = 'PDBFOLDERNAME')
		Begin
			select @lCursor_String = 'INSERT INTO ' + @lDBTargetCab + 'PDBFOLDERNAME(FOLDERINDEX,NAME) Select ' + CONVERT(VARCHAR(10), @lnewFolderIndex)+ ', Name from '+ @lDBsrcLinkName + 'PDBFolderName WHERE FolderIndex = ' + CONVERT(VARCHAR(10),@DBMoveFolderIndex)
			EXECUTE (@lCursor_String)
		END
	...
	BEGIN
		...
		select @DBNewFolderIndex = @DBMoveFolderIndex
		...
		select @lCursor_String = 'SET IDENTITY_INSERT ' + @lDBTargetCab + 'PDBFolder ON 
					...
					ENABLEVERSION, EXPIRYDATETIME, Comment, USEFULDATA, ACL, FINALIZEDFLAG, FINALIZEDDATETIME, FINALIZEDBY, ACLMOREFLAG, MAINGROUPID,
					ENABLEFTS, LOCKMESSAGE, FOLDERLEVEL, OWNERINHERITANCE, EnableSecure) 
					Select ' + CONVERT(VARCHAR(10),@lnewFolderIndex) +', ParentFolderIndex, Name, 1, CreatedDatetime, RevisedDateTime, AccessedDateTime,
					DataDefinitionIndex, AccessType, ImageVolumeIndex, FolderType, FolderLock, LockByUser, Location, DeletedDateTime, EnableVersion,
					ExpiryDateTime, Comment, UseFulData, ACL, FinalizedFlag, FinalizedDateTime, FinalizedBy, ACLMoreFlag, MainGroupId, EnableFTS,
					LockMessage, FolderLevel, OwnerInheritance, EnableSecure FROM ' + @lDBsrcLinkName + 'PDBFolder 
					WHERE FolderIndex = ' + CONVERT(VARCHAR(10),@DBMoveFolderIndex) + 
			...
			BEGIN
				...
				BEGIN
					select @lCursor_String = 'INSERT INTO ' + @lDBTargetCab + 'PDBDOCUMENT (VersionNumber, VersionComment, Name, Owner, CreatedDateTime,
								...
								SELECT VersionNumber, VersionComment, Name, 1, CreatedDateTime, RevisedDateTime,
								AccessedDateTime, DataDefinitionIndex, Versioning, AccessType, DocumentType, CreatedbyApplication, 1, ImageIndex,
								VolumeId, NoOfPages,DocumentSize, FTSDocumentIndex, ODMADocumentIndex, HistoryEnableFlag, DocumentLock, LockByUser, Comment,
								''supervisor'', TextImageIndex, TextVolumeId, FTSFlag, DocStatus, ExpiryDateTime, FinalizedFlag, FinalizedDateTime,
					...
					If Exists (Select * FROM SysObjects (NOLOCK) Where xType = 'U' and upper(name) = 'PDBDOCUMENTNAME')
					Begin
						select @lCursor_String = 'INSERT INTO ' + @lDBTargetCab + 'PDBDOCUMENTNAME(DocumentIndex , Name) Select ' +CONVERT(VARCHAR(10),@newDocumentIndex) + ', Name From ' + @lDBsrcLinkName + 'PDBDOCUMENTName WHERE DocumentIndex = '+ CONVERT(VARCHAR(10),@lTempDocIndex)
						EXECUTE(@lCursor_String)
					END
				...
				BEGIN
					...
					select @lCursor_String = 'SET IDENTITY_INSERT ' + @lDBTargetCab + 'PDBDOCUMENT ON 
								...
								SELECT ' + CONVERT(VARCHAR(10),@newDocumentIndex) + ', VersionNumber, VersionComment, Name, 1, CreatedDateTime,
								RevisedDateTime, AccessedDateTime, DataDefinitionIndex, Versioning, AccessType, DocumentType, CreatedbyApplication,
								1, ImageIndex, VolumeId, NoOfPages,DocumentSize, FTSDocumentIndex, ODMADocumentIndex, HistoryEnableFlag,
								DocumentLock, LockByUser, Comment, ''supervisor'', TextImageIndex, TextVolumeId, FTSFlag, DocStatus, ExpiryDateTime, FinalizedFlag,
								FinalizedDateTime, FinalizedBy, CheckOutstatus, CheckOutbyUser, UseFulData, ACL, PhysicalLocation, ACLMoreFlag, AppName,
				...
				select @lCursor_String = 'INSERT INTO ' + @lDBTargetCab + 'PDBDOCUMENTCONTENT (PARENTFOLDERINDEX, DOCUMENTINDEX, FILEDBY, FILEDDATETIME,
								...
								1, FILEDDATETIME, DOCUMENTORDERNO, REFERECEFLAG, DOCSTATUS FROM ' + @lDBsrcLinkName + 'PDBDOCUMENTCONTENT
								WHERE DocumentIndex = ' + CONVERT(VARCHAR(10),@lTempDocIndex) + ' 
	...
	BEGIN
		...
		BEGIN
			...
			If Exists (Select * FROM SysObjects (NOLOCK) Where xType = 'U' and upper(name) = 'PDBDOCUMENTNAME')
			Begin
				EXECUTE ('DELETE FROM ' + @lDBsrcLinkName + 'PDBDocumentName WHERE DocumentIndex IN ' + @v_docIndexes)	
			End
		...
		If Exists (Select * FROM SysObjects (NOLOCK) Where xType = 'U' and upper(name) = 'PDBFOLDERNAME')
		Begin
			EXECUTE ('DELETE FROM ' + @lDBsrcLinkName + 'PDBFolderName WHERE FolderIndex = ' + @DBMoveFolderIndex)	
		End
