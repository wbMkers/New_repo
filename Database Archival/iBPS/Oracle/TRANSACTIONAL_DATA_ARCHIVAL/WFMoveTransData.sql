...
10/05/2021	Sourabh Tantuway Bug 99342 - iBPS 5.0 SP1: While archiving transactional data, new folderIndex is getting generated for each workitem but , same is not getting updated in ItemIndex and VAR_REC_1
...
IS
  ...
  v_tablevariableremarks:='';
	...
	v_remarks := 'v_sourceCabinet:'|| v_sourceCabinet||',v_targetCabinet:'|| v_targetCabinet||',idblinkname:'|| idblinkname||',tableVariableMap:'|| v_tablevariableremarks||',v_migrateAllProcesses:'   ||v_migrateAllProcesses|| ',v_fromDate:'||v_fromDate||',v_toDate:'||v_toDate ||',v_batchSize:50,v_moveAuditTrailData:'    ||v_moveAuditTrailData||',v_moveFromHistoryData:''N'',v_moveExternalProcessData:'||v_moveExternalProcessData||',v_moveTaskData:'||v_moveTaskData;
	...
	if(v_fromDate is null) then
		SELECT current_timeStamp INTO v_startTime FROM DUAL;	 
		v_query:='Select Min(ENTRYDATETIME) from '|| v_sourcecabinet ||'.WFInstrumentTable'||idblinkname;
		...
		v_dateFilterString:= ' and a.ENTRYDATETIME Between '''||v_defaultfromDate||'''';
    else 
		v_dateFilterString:= ' and a.ENTRYDATETIME Between '''||v_fromDate||'''';
...
'||idblinkname||' where ProcessInstanceId='''||v_processInstanceId ||'''';
		...
		/* Added for Case Management--Archival of task DATA */ 
		  ...
		  ----Custom Data Procedure Start----
		   WFMoveCustomData(v_sourceCabinet,v_targetCabinet,idblinkname,v_processDefId,v_processInstanceId,v_executionLogId,v_loggingEnabled) ; 
		 ...
		 if(v_isDBLink='Y') then
          ...
          MoveDocdb(v_sourcecabinet,v_targetCabinet,v_isDBLink, v_ODDBLink,TO_NUMBER(v_folderIndex), 'Y','Y','Y',v_genIndex,v_folderStatus);
          IF (v_folderStatus <> 0) THEN
            ROLLBACK;
            dbms_output.put_line('Error code while migrating folder '||v_folderIndex|| 'is ' || v_folderStatus);
            RETURN;
          END IF;
		  update PDBFolder set ParentFolderIndex=v_parentfolderid where FolderIndex=v_genIndex;
		  
		
		IF((v_variantId =0) AND (v_moveExternalProcessData='Y')) THEN
			IF(v_externalTableCount>0 and v_genIndex >0 ) THEN
			
			v_query :=	'SELECT COLUMN_NAME FROM DBA_TAB_COLUMNS WHERE UPPER(TABLE_NAME) = UPPER('''||v_externalTable||''') AND OWNER = UPPER(''' || v_targetCabinet || ''')';
        	OPEN v_ColumnCursor FOR TO_CHAR(v_query);
			  v_colStrExt := '';
			  LOOP
				FETCH v_ColumnCursor INTO v_columnName;
			  EXIT
			WHEN v_ColumnCursor%NOTFOUND;
			  IF(Upper(v_columnName) <> 'ITEMINDEX') then
				v_colStrExt := v_colStrExt ||v_columnName||',';
			  END IF ;
			END LOOP;
			--v_colStrExt:=trim(trailing ',' FROM v_colStr);
			CLOSE v_ColumnCursor;

            v_query := 'Insert into '||v_targetCabinet||'.'||v_externalTable||'('||v_colStrExt||'ITEMINDEX) Select '||v_colStrExt||''||v_genIndex||' from '||v_sourceCabinet||'.'||v_externalTable||''||idblinkname||' where  ItemIndex = '''||v_folderIndex||'''' ;
		    ...
		    v_executionTime:=v_endTime-v_startTime;
			IF(v_loggingEnabled='Y') THEN
				INSERT INTO WFQueryLogTable (ExecutionLogId,query,queryExecutionDateTime,executionTime) VALUES(v_executionLogId,SUBSTR(v_query,1,4000),v_endTime,v_executionTime);
			END IF;
            v_query := 'Delete from '||v_sourceCabinet||'.'||v_externalTable||idblinkname||' where  ItemIndex = '''||v_folderIndex ||'''';
            SELECT current_timeStamp INTO v_startTime FROM DUAL;	
            EXECUTE Immediate v_query ;
			...
			v_query  := 'Update ' || v_targetCabinet || '.QueueHistoryTable set VAR_REC_1 = ' || v_genIndex || ' where   ProcessInstanceId ='''|| v_processInstanceId ||'''';
			SELECT current_timeStamp INTO v_startTime FROM DUAL;	
            EXECUTE Immediate v_query ;
			SELECT current_timeStamp INTO v_endTime FROM DUAL; 
		    v_executionTime:=v_endTime-v_startTime;
