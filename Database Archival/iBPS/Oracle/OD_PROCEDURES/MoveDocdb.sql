...
BEGIN
	...
	lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBFOLDER (FOLDERINDEX, PARENTFOLDERINDEX, NAME, OWNER, CREATEDDATETIME, REVISEDDATETIME,
					...
					Select ' || lnewFolderIndex  ||', ParentFolderIndex, Name, 1, CreatedDatetime, RevisedDateTime, AccessedDateTime,
					...
					WHERE FolderIndex = ' || lTempOldFoldId;
	EXECUTE IMMEDIATE lCursor_String;
	
	v_existsFlag :=0;
	SELECT 1 INTO v_existsFlag FROM user_tables WHERE UPPER(TABLE_NAME)=UPPER('PDBFOLDERNAME') ;
	IF v_existsFlag = 1 THEN
	BEGIN
		lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBFOLDERNAME(FOLDERINDEX,NAME) Select ' || lnewFolderIndex  ||', Name FROM ' || DBSourceCabinet || '.PDBFolderName'||lDBsrcLinkName ||' WHERE FolderIndex = ' || lTempOldFoldId;
	...
	THEN
		...
		LOOP
			...
			lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBDOCUMENT (DocumentIndex, VersionNumber, VersionComment, Name, Owner, CreatedDateTime,
							...
							SELECT ' ||	newDocumentIndex ||', VersionNumber, VersionComment, Name, 1, CreatedDateTime, RevisedDateTime,
			...
			v_existsFlag :=0;
			SELECT 1 INTO v_existsFlag FROM user_tables WHERE UPPER(TABLE_NAME)=UPPER('PDBDOCUMENTNAME') ;
			IF v_existsFlag = 1 THEN
			BEGIN
				lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBDOCUMENTNAME(DocumentIndex,Name) SELECT ' ||	newDocumentIndex ||',Name FROM ' || DBSourceCabinet || '.PDBDOCUMENTNAME'||lDBsrcLinkName ||'  
							WHERE DocumentIndex = '|| lTempDocIndex;
			END;
			END IF;
				
			lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBDOCUMENTCONTENT (PARENTFOLDERINDEX, DOCUMENTINDEX, FILEDBY, FILEDDATETIME,
							DOCUMENTORDERNO, REFERECEFLAG, DOCSTATUS) SELECT ' ||	lnewFolderIndex ||',' || newDocumentIndex || ', 1, FILEDDATETIME,
	...
	THEN
		...
		THEN
			...
			IF v_existsFlag = 1 THEN
			BEGIN
				EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBDOCUMENTNAME'||lDBsrcLinkName ||' WHERE DocumentIndex IN ' || v_docIndexes);
			END;
			END IF;
	...
	/*	EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBFolderContent'||lDBsrcLinkName ||' WHERE FolderIndex = ' || lTempOldFoldId ); 
		...
		IF v_existsFlag = 1 THEN
		BEGIN
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBFolderName'||lDBsrcLinkName ||' WHERE FolderIndex = ' || lTempOldFoldId ); 
		END;
		END IF;
