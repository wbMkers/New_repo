  ...
  BEGIN
    BEGIN
		...
		ELSE
			...
			END IF;
		END IF;
		  v_queryFolder:='select RouteFolderId,ScratchFolderId,WorkFlowFolderId,CompletedFolderId,DiscardFolderId,ProcessDefId from '|| v_sourcecabinet||'.RouteFolderDefTable'|| idblinkname||v_FilterQueryString;
...
BEGIN
			...
			dbms_output.put_line('   '||v_routeFolderId );
				MoveDocdb(v_sourcecabinet,v_targetCabinet,v_isDBLink, v_localdblink,TO_NUMBER(v_routeFolderId), 'N','Y','N',v_genIndex,v_folderStatus);
        ...
        RAISE ex_custom;
				END IF;
				UPDATE RouteFolderDefTable SET RouteFolderId = v_genIndex WHERE ProcessDefId = v_pid;
				v_parentFolderId := v_genIndex;
				/*MoveDocdb(v_sourcecabinet,v_targetCabinet,v_isDBLink, v_localdblink,TO_NUMBER(v_scratchFolderId), 'N','Y','N',v_genIndex,v_folderStatus);
				IF (v_folderStatus <> 0) THEN
         v_errorFolder:= v_scratchFolderId;
          ...
          RAISE ex_custom;
				END IF; */
				MoveDocdb(v_sourcecabinet,v_targetCabinet,v_isDBLink, v_localdblink,TO_NUMBER(v_workFlowFolderId), 'N','Y','N',v_genIndex,v_folderStatus);
				IF (v_folderStatus <> 0) THEN
        v_errorFolder:= v_workFlowFolderId;
         ...
         RAISE ex_custom;
				END IF;
				UPDATE RouteFolderDefTable SET WorkFlowFolderId = v_genIndex WHERE ProcessDefId = v_pid;
					UPDATE PDBFolder SET ParentFolderIndex = v_parentFolderId WHERE FolderIndex = v_genIndex;
				/* MoveDocdb(v_sourcecabinet,v_targetCabinet,v_isDBLink, v_localdblink,TO_NUMBER(v_completedFolderId), 'N','Y','N',v_genIndex,v_folderStatus);
				IF (v_folderStatus <> 0) THEN
        v_errorFolder:= v_completedFolderId;
				  ...
				  EXIT;
				END IF;
				MoveDocdb(v_sourcecabinet,v_targetCabinet, v_isDBLink,v_localdblink,TO_NUMBER(v_dicarderFolderId),'N', 'Y','N',v_genIndex,v_folderStatus);
