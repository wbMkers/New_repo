...
Module                      : IBPS Server;
File Name                   : WF_DATABASE_PARTITIONING.sql (Oracle)
...
***************************************************************************************/
create or replace
PROCEDURE WFPartitionCabinetTables
  (
...
END;';
	...
	v_query    := 'SELECT * FROM ' || v_targetCabinet || '.WFPARTITIONTABLELIST where partitionType=''R''';
	BEGIN
	  ...
	  WHEN v_TableNameCursor%NOTFOUND;
		...
		IF (v_existsFlag = 1) THEN
		  WFPartitionGlobalTable(v_targetCabinet, v_tableName,v_tableAlias);
		END IF;
	  END LOOP;
