...
BEGIN
	...
	THEN
		...
		LOOP
			...
			lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBINTGLOBALINDEX (DATAFIELDINDEX, DOCUMENTINDEX, INTVALUE)
							   SELECT DATAFIELDINDEX, ' || newDocumentIndex || ', INTVALUE 
							   FROM ' || DBSourceCabinet || '.PDBINTGLOBALINDEX'||lDBsrcLinkName ||
							   ' WHERE DocumentIndex = '||lTempDocIndex; 
			EXECUTE IMMEDIATE lCursor_String;
			
			lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBBOOLGLOBALINDEX (DATAFIELDINDEX, DOCUMENTINDEX, BOOLVALUE)
							  SELECT DATAFIELDINDEX, ' || newDocumentIndex || ', BOOLVALUE 
							  FROM ' || DBSourceCabinet || '.PDBBOOLGLOBALINDEX'||lDBsrcLinkName ||
							  ' WHERE DocumentIndex = '||lTempDocIndex; 
			EXECUTE IMMEDIATE lCursor_String;
			
			lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBFLOATGLOBALINDEX (DATAFIELDINDEX, DOCUMENTINDEX, FLOATVALUE)
							  SELECT DATAFIELDINDEX, ' || newDocumentIndex || ', FLOATVALUE 
							  FROM ' || DBSourceCabinet || '.PDBFLOATGLOBALINDEX'||lDBsrcLinkName ||
							  ' WHERE  DocumentIndex = '||lTempDocIndex; 
			EXECUTE IMMEDIATE lCursor_String;
			
			lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBDATEGLOBALINDEX (DATAFIELDINDEX, DOCUMENTINDEX, DATEVALUE)
							  SELECT DATAFIELDINDEX, ' || newDocumentIndex || ', DATEVALUE 
							  FROM ' || DBSourceCabinet || '.PDBDATEGLOBALINDEX'||lDBsrcLinkName ||
							  ' WHERE  DocumentIndex = '||lTempDocIndex; 
			EXECUTE IMMEDIATE lCursor_String;
			
			lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBSTRINGGLOBALINDEX (DATAFIELDINDEX, DOCUMENTINDEX, STRINGVALUE)
							  SELECT DATAFIELDINDEX, ' || newDocumentIndex || ', STRINGVALUE 
							  FROM ' || DBSourceCabinet || '.PDBSTRINGGLOBALINDEX'||lDBsrcLinkName ||
							  ' WHERE  DocumentIndex = '||lTempDocIndex; 
			EXECUTE IMMEDIATE lCursor_String;	 
			
			lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBLONGGLOBALINDEX (DATAFIELDINDEX, DOCUMENTINDEX, LONGVALUE)
							  SELECT DATAFIELDINDEX, ' || newDocumentIndex || ', LONGVALUE 
			...
			OPEN RC1 FOR 
				...
				FROM ' || DBSourceCabinet || '.PDBTEXTGLOBALINDEX'||lDBsrcLinkName || ' WHERE  DocumentIndex = '||lTempDocIndex;
			LOOP 
				FETCH RC1 INTO lDataFieldIndex, lTextValue; 
				EXIT WHEN RC1%NOTFOUND; 
				lCursor_String := 'INSERT INTO ' || DBTargetCabinet ||'.PDBTEXTGLOBALINDEX(DataFieldIndex, DocumentIndex, TextValue) 
								VALUES(' || lDataFieldIndex ||','|| newDocumentIndex ||','|| lTextValue ||')'; 
				EXECUTE IMMEDIATE lCursor_String;	
			END LOOP; 
			CLOSE RC1;
	...
	THEN
		...
		THEN
			...
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBIntGlobalindex'||lDBsrcLinkName ||' WHERE DocumentIndex IN  ' || v_docIndexes ); 
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBBoolGlobalindex'||lDBsrcLinkName ||' WHERE DocumentIndex IN  ' || v_docIndexes ); 
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBFloatGlobalindex'||lDBsrcLinkName ||' WHERE  DocumentIndex IN  ' || v_docIndexes ); 
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBDateGlobalindex'||lDBsrcLinkName ||' WHERE DocumentIndex IN  ' || v_docIndexes ); 
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBStringGlobalindex'||lDBsrcLinkName ||' WHERE DocumentIndex IN  ' || v_docIndexes ); 
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBLongGlobalIndex'||lDBsrcLinkName ||' WHERE  DocumentIndex IN  ' || v_docIndexes ); 
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBDocIdGlobalIndex'||lDBsrcLinkName ||' WHERE  DocumentIndex IN  ' || v_docIndexes ); 
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBTextGlobalIndex'||lDBsrcLinkName ||' WHERE   DocumentIndex IN  ' || v_docIndexes ); 
			...
			EXECUTE IMMEDIATE(' DELETE FROM '|| DBSourceCabinet || '.PDBReminder'||lDBsrcLinkName ||' WHERE DocIndex IN  ' || v_docIndexes); 
