...
//		File Name					: WFTimerClientServiceHandler.java
//		Author						: Mandeep Kaur
//		Date written (DD/MM/YYYY)	: 30/08/2005
//		Description					: ServiceHandler for WFTimerBeans
//-------------------------------------------------------------------------
//					CHANGE HISTORY
//-------------------------------------------------------------------------
...
public class WFTimerClientServiceHandlerBean extends NGOServerInterface {

	private static Object customTransactionHome = null;
	private static final ArrayList<String> transactionFreeCalls = new ArrayList<String>();
	...
	static {
		...
		// Add the transaction free calls in array list 
        transactionFreeCalls.add("WFGetServicesStatusForMonitor".toUpperCase());
		try{
	...
	public String execute(String strInputXML) throws JTSException {
		...
		try {
			...
			if (isTransactionFreeCall(strOption)) {
                strReturn = execute(strOption, "", null, inputXml, new XMLGenerator());
            } else {
			...
			if((nge.getCategory()).equals("1"))
				throw new JTSException("1", nge.getMessage(), ""+ nge.getErrorCode(), "WFTimerClientServiceHandler : "+strOption,"");
			else
				throw new JTSException("4", nge.getMessage(), ""+ nge.getErrorCode(), "WFTimerClientServiceHandler : "+strOption,"");
		} catch(Exception e){
			WFSUtil.printErr(strCabinetName,"", e);
			throw new JTSException("4", e.getMessage(),"-94001", "WFTimerClientServiceHandler : "+strOption,"");
		} finally {
			try {
				if(conn != null){
					if (!conn.getAutoCommit()){
            ...
            try{
				...
				if(WFTransactionFree.isTransactionFreeCall(strOption)){
					if(outputXml!=null){
                                WFSUtil.writeLog("WFTimerClientServiceHandlerBean", strOption, startTime, endTime, error,outputXml.toString());
					}else{
						WFSUtil.writeLog("WFTimerClientServiceHandlerBean", strOption, startTime, endTime, error,"");
					}
				WFSUtil.writeLog(strInputXML, strReturn);
                                ...
                                {
                                	if(outputXml!=null){
                                WFSUtil.writeLog("WFTimerClientServiceHandlerBean", strOption, startTime, endTime, error,outputXml.toString(),strCabinetName, true,(connectionEndTime-startTime),sessionID, userID); /*Bug 38914 */
                                	}else{
                                		 WFSUtil.writeLog("WFTimerClientServiceHandlerBean", strOption, startTime, endTime, error,"",strCabinetName, true,(connectionEndTime-startTime),sessionID, userID);
                                	}
	...
	public Object getHomeObject(String strOption, Context ctx, String appName, String engineName){
		Object home = null;
		String lookupName = "";
        lookupName = "java:comp/env/" + "WFServiceManagerBean";
		try {
			if (customTransactionHome ==  null){
				...
				customTransactionHome = PortableRemoteObject.narrow(home, Class.forName("com.newgen.omni.jts.txn.WFTimerTransactionHome"));
			}
		}
		catch (Exception e) {
	...
	public String execute(String option, String engine, java.sql.Connection con, XMLParser parser, XMLGenerator gen) throws Exception {
        String strReturn = "";
		String AppName="WFServiceManagerBean";
		EJBLocalObject JTSTxn	= null;
		String toLookUp =  option.trim();
		Object obj = getHomeObject(toLookUp, ctx,AppName, engine);
		Object txnTemp = Class.forName("com.newgen.omni.jts.txn.WFTimerTransactionHome").getMethod("create", new Class[] {}).invoke(obj, new Object[] {});
		JTSTxn = (EJBLocalObject)PortableRemoteObject.narrow(txnTemp, Class.forName("com.newgen.omni.jts.txn.WFTimerTransaction"));
		strReturn = (String) Class.forName("com.newgen.omni.jts.txn.WFTimerTransaction").getMethod("execute", new Class[]
    ...
    private boolean isTransactionFreeCall(String txnName) {
        return transactionFreeCalls.contains(txnName.toUpperCase());
    }

} //end of WFTimerClientServicehandlerBean class
