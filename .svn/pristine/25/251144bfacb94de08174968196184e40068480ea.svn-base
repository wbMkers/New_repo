...
______________________________________________________________________________________________________
	...
	File Name			: WFReturnRightsForUser.sql (Oracle)
	Author				: Mohnish Chopra
	Date written (DD/MM/YYYY)	: 04/02/2014
...
CREATE OR REPLACE PROCEDURE WFReturnRightsForUser(
	temp_v_DBUserId				INT, 		
...
/*	IF(v_DBlastValue IS NOT NULL) THEN
	BEGIN 
		v_lastValueStr := ' AND ObjectName ' || v_op || v_quoteChar || v_DBlastValue || v_quoteChar;
	END;
	END IF;
*/	
	...
	BEGIN		
		...
		v_DBFilterString:=REPLACE(v_DBFilterString,v_quoteChar||v_quoteChar,v_quoteChar);
		*/
	...
	IF(v_queryParam0 IS NOT NULL AND v_queryParam1 IS NOT NULL AND v_queryParam2 IS NOT NULL) THEN
		...
		A.AssignedTillDATETIME >=SYSDATE) ' || v_queryParam2 ||v_ProjectIdCondition; 
	ELSE 
		...
		A.AssignedTillDATETIME >=SYSDATE) ' ||v_ProjectIdCondition; 
	END IF;
...
/*	v_queryStr := 'SELECT ObjectId, ObjectName FROM (Select DISTINCT ObjectId, ObjectName from ' 
	...
	v_queryStr := 'SELECT ObjectId, ObjectName FROM (Select DISTINCT ObjectId, ObjectName from ' 
	|| v_TempTableName || ' WHERE 1 = 1 ' || v_filterStr || v_orderByStr || ')';
