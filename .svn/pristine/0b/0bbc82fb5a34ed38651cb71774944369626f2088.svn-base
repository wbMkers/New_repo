	...
	BEGIN
		...
		DECLARE @v_folderIndex 				VARCHAR(256)
		...
		BEGIN
			...
			BEGIN
				...
				Begin
				EXEC SetComplexDataArchival @v_ProcessDefId, @dblinkString,'N'
				END
				...
				BEGIN
					...
					SELECT @v_CountQuery = ' SELECT @value = COUNT(TABLENAME) FROM ' + @dblinkString + '.EXTDBCONFTABLE  WHERE PROCESSDEFID = ' +  CONVERT(NVarchar(10),@v_ProcessDefId) + 'AND EXTOBJID = 1'
					SELECT @v_queryParam = '@value INT OUTPUT'
					EXEC sp_executesql @v_CountQuery, @v_queryParam, @value = @v_externalTableCount OUTPUT
				END
				...
				BEGIN
					...
					SELECT @v_query = 'SELECT TOP 50 A.PROCESSINSTANCEID , A.VAR_REC_1 FROM ' + @dblinkString + '.' + 'WFINSTRUMENTTABLE A WITH (NOLOCK)  WHERE A.PROCESSDEFID = ' + CONVERT (NVARCHAR(10),@v_ProcessDefId) + @variantFilterString + 
					' AND A.ENTRYDATETIME > CONVERT(DATETIME, ' + '''' + @v_fromDate + '''' + ')'+' AND  A.ENTRYDATETIME < CONVERT( DATETIME,' + '''' + @v_toDate + +''''+')
					...
					BEGIN TRANSACTION MOVE_TRANS_DATA ------------------------------Move Trans Data Begins Here
						...
						BEGIN
							...
							BEGIN	
									...
									SELECT @v_query = 'Insert into QUEUEHISTORYTABLE' + '(' + @v_colName + ')' + ' Select ' + REPLACE(@v_colName, 'VAR_REC_1', @newFolderIndex) + ' from ' +  @dblinkString + '.WFINSTRUMENTTABLE where processinstanceid = '+''''+ @v_processInstanceId + ''''
									PRINT @v_query
									EXECUTE(@v_query)
					...
					/* Added for Case Management--Archival of task DATA */ 
									...
									BEGIN
										...
										SELECT @v_query = 'Insert into WFTaskPreCheckHistory Select * from ' +  @dblinkString + '.WFTaskPreCheckTable where processinstanceid = ' +''''+ @v_processInstanceId + ''''
										EXECUTE(@v_query)
					...
					/* End Of Case Management--Archival of task DATA */ 
									...
									EXEC WFMoveCustomData @v_sourcecabinet,@v_targetCabinet,@dblinkString,@v_ProcessDefId ,@v_processInstanceId,@v_DeleteFromSrc,@executionLogId
									IF(@@error <> 0)
									Begin										
										CLOSE v_cursor 
										DEALLOCATE v_cursor 
