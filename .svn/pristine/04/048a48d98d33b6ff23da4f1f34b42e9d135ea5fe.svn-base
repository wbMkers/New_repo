	...
	BEGIN
		...
		SELECT @v_query = ' Insert into WFHistoryRouteLogTable ( ' + @colStr +' ) Select '+ @colStr + ' from ' +  @dblinkString + '.WFCurrentRouteLogTable where processinstanceid = '+''''+ @v_processInstanceId + ''''
		EXECUTE(@v_query)
		...
		SELECT @v_query = ' Insert into WFATTRIBUTEMESSAGEHISTORYTABLE ( ' + @colStr +' ) Select '+ @colStr + ' from ' +  @dblinkString + '.WFATTRIBUTEMESSAGETABLE where processinstanceid = '+''''+ @v_processInstanceId + ''''
		EXECUTE(@v_query)
