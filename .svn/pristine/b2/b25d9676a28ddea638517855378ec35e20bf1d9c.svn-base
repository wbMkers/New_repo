...
____________________________________________________________________________________*/
create or replace Function WFMigrateMetaData
  (
...
AS
  ...
  v_queryFolder  VARCHAR(4000);
  v_queryStr2        VARCHAR(4000);
  v_tableId      INTEGER;
  v_variantId    INTEGER;
  v_tableName    VARCHAR(256);
  v_routeFolderId VARCHAR(255);
  v_scratchFolderId VARCHAR(255);
  v_workFlowFolderId VARCHAR(255);
  v_completedFolderId VARCHAR(255);
  v_dicarderFolderId VARCHAR(255);
  ...
  v_tablevariableremarks VARCHAR(4000);
  v_TableNameCursor refcursor;

    v_processDefIdCursor refcursor;
     v_importedFlag     INT;
    v_migratedflag     INT;
    v_existsFlag       INT;
...
END;';
    ...
    BEGIN
      ...
      ELSE -- Not yet Migrated
		...
		BEGIN
          ...
          EXECUTE v_querystr2 INTO v_existsflag;
        EXCEPTION
        WHEN OTHERS THEN
          v_existsflag := 0;
        END;
    ...
    SELECT WFChangeTriggersStatus(v_sourceCabinet,v_targetCabinet,'E',idblinkname);
