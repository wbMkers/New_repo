...
BEGIN
   ...
   (
	...
	BEGIN
		v_existFlag := 0;
		SELECT 1 INTO v_existFlag FROM information_schema.columns WHERE UPPER(table_name) = UPPER('WFBRMSConnectTable') and UPPER(column_name)=UPPER('RESTServerHostName');
		IF(NOT FOUND) THEN 
			Execute ('Alter table WFBRMSConnectTable ADD RESTServerHostName  varchar(128) ');
		END IF;
	END;
	...
	BEGIN
		v_existFlag := 0;
		SELECT 1 INTO v_existFlag FROM information_schema.columns WHERE UPPER(table_name) = UPPER('WFBRMSConnectTable') and UPPER(column_name)=UPPER('RESTServerPort');
		IF(NOT FOUND) THEN 
	...
	BEGIN
		v_existFlag := 0;
		SELECT 1 INTO v_existFlag FROM information_schema.columns WHERE UPPER(table_name) = UPPER('WFBRMSConnectTable') and UPPER(column_name)=UPPER('RESTServerProtocol');
		IF(NOT FOUND) THEN 
			Execute ('Alter table WFBRMSConnectTable ADD RESTServerProtocol  varchar(32) ');
		END IF;
	END;
	...
	BEGIN
		v_existFlag := 0;
		select character_maximum_length into v_existFlag from information_schema.columns WHERE UPPER(table_name) = UPPER('WFMAILQUEUETABLE') and UPPER(column_name)=UPPER('mailsubject');
		IF(v_existFlag<512) THEN
			EXECUTE('ALTER TABLE wfmailqueuetable Alter COLUMN mailSubject TYPE VARCHAR(512)');
		end if;
	END;
	
	BEGIN
		v_existFlag := 0;
		select character_maximum_length into v_existFlag from information_schema.columns WHERE UPPER(table_name) = UPPER('WFMAILQUEUEHISTORYTABLE') and UPPER(column_name)=UPPER('mailsubject');
		IF(v_existFlag<512) THEN
			EXECUTE('ALTER TABLE wfmailqueuehistorytable Alter COLUMN mailSubject TYPE VARCHAR(512)');
		end if;
	END;
