...
CREATE OR REPLACE PROCEDURE Upgrade AS
	...
	v_logStatus 			BOOLEAN;
	v_scriptName            NVARCHAR2(100) := 'Upgrade09_SP00_006';
...
BEGIN
	v_logStatus := LogInsert(1,'Creating VIEW QUSERGROUPVIEW ');
	BEGIN
    v_logStatus := LogSkip(1);
    EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW QUSERGROUPVIEW 
		AS	SELECT * FROM ( SELECT queueid, userid, cast ( NULL AS integer ) AS GroupId, assignedtilldatetime, queryFilter
				FROM QUEUEUSERTABLE 
	...
	v_logStatus := LogInsert(2,'Creating VIEW WFUSERVIEW');
	BEGIN
    v_logStatus := LogSkip(2);
    EXECUTE IMMEDIATE '	CREATE OR REPLACE VIEW WFUSERVIEW ( USERINDEX, USERNAME, PERSONALNAME, FAMILYNAME, CREATEDDATETIME, EXPIRYDATETIME, PRIVILEGECONTROLLIST, PASSWORD, ACCOUNT, COMMNT, DELETEDDATETIME, USERALIVE, MAINGROUPID, MAILID, FAX, NOTECOLOR )
		AS 
	...
	v_logStatus := LogInsert(3,'Creating VIEW WFSESSIONVIEW');
	BEGIN
      v_logStatus := LogSkip(3);
      EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW WFSESSIONVIEW ( SESSIONID,USERID, USERLOGINTIME, SCOPE, MAINGROUPID, 
