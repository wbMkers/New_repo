...
BEGIN 
			...
			IF NOT EXISTS(SELECT * FROM WFCABVERSIONTABLE WHERE CABVERSION IN ('7.1','8.0'))
			BEGIN
				DECLARE activityExpiryCur Cursor FASt_Forward For SELECT processDefId , activityId , Expiry FROM ActivityTABLE Where Expiry is not null 
...
BEGIN 
		...
		SELECT @v_scriptName = 'Upgrade09_SP00_007'
				
		
	
exec @v_logStatus= LogInsert 1,@v_scriptName,'Updating WFCabVersionTable and executing procedures i.e MultipleExcpProc,ProcessTATProc,ActivityTATProc,ActivityExpiryProc,EscalationProc '
	BEGIN
	BEGIN TRY
