CREATE OR ALTER PROCEDURE PMW_COMPLX_DELETE_NEW(
...
BEGIN
	...
	BEGIN
		Execute ('ALTER TABLE WFINSTRUMENTTABLE ADD COMPLEX_FLAG NVARCHAR(4)');
	END
	...
	WHILE @CURRENT_COUNT < @TOTAL_ROW
		BEGIN
			...
			SET @IN_PROCESSINSTANCEID = 'SELECT TOP 100 ProcessInstanceId FROM WFINSTRUMENTTABLE WITH (NOLOCK) WHERE PROCESSDEFID = ' + CONVERT(NVARCHAR,@IN_PROCESSDEFID) + ' AND ENTRYDATETIME < GETDATE() - ' + CONVERT(NVARCHAR,@IN_NOOFDAYS) + ' AND COMPLEX_FLAG IS NULL'
			...
			END CATCH
			/*DELETE FROM CSRF_LIstBox WHERE MAPID IN (SELECT ProcessInstanceId 
			FROM WFINSTRUMENTTABLE WITH (NOLOCK) WHERE ProcessInstanceId IN ( SELECT TOP 2 ProcessInstanceId FROM WFINSTRUMENTTABLE WITH (NOLOCK) 
			WHERE PROCESSDEFID = 10 AND ENTRYDATETIME < GETDATE() - 0  AND COMPLEX_FLAG IS NULL))
		
			UPDATE WFINSTRUMENTTABLE
			SET COMPLEX_FLAG='N'
			WHERE ProcessInstanceId IN ( SELECT TOP 2 ProcessInstanceId FROM WFINSTRUMENTTABLE WITH (NOLOCK) 
			WHERE PROCESSDEFID = 10 AND ENTRYDATETIME < GETDATE() - 0  AND COMPLEX_FLAG IS NULL)
			*/
		
			--PRINT ('delete from 3-->'+@V_QUERYSTR_NEW)
			BEGIN TRY
				SET @UPDATE_STR = 'UPDATE WFINSTRUMENTTABLE SET COMPLEX_FLAG=''N'' WHERE ProcessInstanceId IN ( ' + @IN_PROCESSINSTANCEID + ')'
	...
	ELSE
		...
		END
	IF EXISTS(SELECT * FROM sysColumns WHERE id = (SELECT id FROM sysObjects WHERE NAME = 'WFINSTRUMENTTABLE') AND  NAME = 'COMPLEX_FLAG')
	BEGIN
		Execute ('ALTER TABLE WFINSTRUMENTTABLE DROP COLUMN COMPLEX_FLAG');
	END
