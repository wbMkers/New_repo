...
IS
  ...
  BEGIN
	...
	WHILE(1=1)
      LOOP
	v_query    := 'Select * from(SELECT a.parentprocessinstanceid FROM '|| v_sourcecabinet ||'.WFLinksTable'||idblinkname||' a WHERE a.ParentProcessInstanceId >'''|| v_lastInstanceId|| ''' order by a.ParentProcessInstanceId ) where ROWNUM <=100';
    v_queryStr := 'Select count(*) from(SELECT a.parentprocessinstanceid FROM '|| v_sourcecabinet ||'.WFLinksTable'||idblinkname||' a WHERE a.ParentProcessInstanceId >'''|| v_lastInstanceId|| ''' order by a.ParentProcessInstanceId ) where ROWNUM <=100';
    ...
    END IF;
		...
		v_query := 'INSERT INTO ' || v_targetCabinet || '.WFLinksTable SELECT * FROM ' || v_sourceCabinet || '.WFLinksTable'||idblinkname|| '  WHERE ParentProcessInstanceId ='''||v_processInstanceId ||'''';
		SELECT current_timeStamp INTO v_startTime FROM DUAL;
		EXECUTE IMMEDIATE v_query;
		SELECT current_timeStamp INTO v_endTime FROM DUAL; 
		 v_executionTime:=v_endTime-v_startTime;
